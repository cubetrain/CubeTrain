...
namespace cubetrain { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(cubetrain);
const static uint64_t null_account_name      = N(cubetrain.null);
const static uint64_t producers_account_name = N(cubetrain.prods);
...
const static uint64_t cubetrain_auth_scope       = N(cubetrain.auth);
const static uint64_t cubetrain_all_scope        = N(cubetrain.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t cubetrain_any_name = N(cubetrain.any);
const static uint64_t cubetrain_code_name = N(cubetrain.code);
...
const static cubetrain::chain::wasm_interface::vm_type default_wasm_runtime = cubetrain::chain::wasm_interface::vm_type::binaryen;
...
} } } // namespace cubetrain::chain::config

constexpr uint64_t SEAT_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / cubetrain::chain::config::percent_100;
}

template<typename Number>
Number SEAT_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + cubetrain::chain::config::percent_100 - cubetrain::chain::config::percent_1)  / cubetrain::chain::config::percent_100;
}
