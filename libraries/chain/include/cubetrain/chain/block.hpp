#pragma once
#include <cubetrain/chain/block_header.hpp>
...
FC_REFLECT_ENUM( cubetrain::chain::transaction_receipt::status_enum,
                 (executed)(soft_fail)(hard_fail)(delayed)(expired) )

FC_REFLECT(cubetrain::chain::transaction_receipt_header, (status)(cpu_usage_us)(net_usage_words) )
FC_REFLECT_DERIVED(cubetrain::chain::transaction_receipt, (cubetrain::chain::transaction_receipt_header), (trx) )
FC_REFLECT_DERIVED(cubetrain::chain::signed_block, (cubetrain::chain::signed_block_header), (transactions)(block_extensions) )
