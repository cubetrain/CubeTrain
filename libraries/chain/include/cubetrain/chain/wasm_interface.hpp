...
namespace cubetrain { namespace chain {
   ...
   namespace webassembly { namespace common {
      ...
      try {
         ...
         if(validating && mod_name != "env")
            SEAT_ASSERT( false, wasm_exception, "importing from module that is not 'env': ${module}.${export}", ("module",mod_name)("export",export_name) );
         ...
         SEAT_ASSERT( false, wasm_exception, "${module}.${export} unresolveable", ("module",mod_name)("export",export_name) );
         return false;
   ...
   class wasm_interface {
      public:
         ...
         //validates code -- does a WASM validation pass and checks the wasm against CUBETRAIN specific constraints
      ...
      private:
         ...
         friend class cubetrain::chain::webassembly::common::intrinsics_accessor;
   };
...
FC_REFLECT_ENUM( cubetrain::chain::wasm_interface::vm_type, (wavm)(binaryen) )
