...
#define N(X) cubetrain::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<cubetrain::chain::name> : private hash<uint64_t> {
      typedef cubetrain::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const cubetrain::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, cubetrain::chain::name& check);
} // fc


FC_REFLECT( cubetrain::chain::name, (value) )
