<template>
  <div id="app">
    <img src="@/assets/seat-logo.png" style="height:200px;">
    ...
    <div style="margin-top: 20px;">
      ...
      <p v-if="showPublicTips"><span>验证SEAT公钥：</span>{{ valiPublicStatus  ? '正确' : '错误！'}}</p>
      <p>（验证SEAT公钥格式）</p>
    </div>
    ...
    <div style="margin-top: 20px;">
      ...
      <p v-if="showPrivateTips"><span>验证SEAT私钥：</span>{{ valiPrivateStatus ? '正确' : '错误！'}}</p>
      <p>（验证SEAT私钥格式）</p>
    </div>
  ...
  methods: {
    ...
    getPrivateKey () {
      ecc.randomKey().then(privateKey => {
        ...
        this.publicKey = 'SEAT' + ecc.privateToPublic(this.privateKey).slice(3)
        this.show = true
      })
    },
    getPrivateKeyBySeed () {
    ...
    isValidPublic () {
      ...
      this.valiPublicStatus = ecc.isValidPublic('SEAT' + this.valiPublicKey.slice(4))
    },
    isValidPrivate () {
