...
class currency_tester : public TESTER {
   public:
      ...
      asset get_balance(const account_name& account) const {
         return get_currency_balance(N(cubetrain.tk), symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action(N(cubetrain.tk), N(issue), mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      currency_tester()
      :TESTER(),abi_ser(json::from_string(cubetrain_tk_abi).as<abi_def>(), abi_serializer_max_time)
      {
         create_account( N(cubetrain.tk));
         set_code( N(cubetrain.tk), cubetrain_tk_wast );

         auto result = push_action(N(cubetrain.tk), N(create), mutable_variant_object()
                 ("issuer",       cubetrain_tk)
         ...
         result = push_action(N(cubetrain.tk), N(issue), mutable_variant_object()
                 ("to",       cubetrain_tk)
      ...
      static const std::string cubetrain_tk;
};

const std::string currency_tester::cubetrain_tk = name(N(cubetrain.tk)).to_string();
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   auto actual = t.get_currency_balance(N(cubetrain.tk), expected.get_symbol(), N(cubetrain.tk));
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      auto trace = push_action(N(cubetrain.tk), N(transfer), mutable_variant_object()
         ("from", cubetrain_tk)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   BOOST_REQUIRE_THROW(push_action(N(cubetrain.tk), N(transfer), mutable_variant_object()
                                    ("from", cubetrain_tk)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
...
BOOST_FIXTURE_TEST_CASE( test_overspend, currency_tester ) try {
   ...
   // Overspend!
   {
      ...
      BOOST_CHECK_EXCEPTION( push_action(N(alice), N(transfer), data),
                             cubetrain_assert_message_exception, cubetrain_assert_message_is("overdrawn balance") );
      produce_block();
...
BOOST_FIXTURE_TEST_CASE(test_symbol, TESTER) try {
   ...
   {
      symbol sys(4, "SEAT");
      BOOST_REQUIRE_EQUAL(SY(4,SEAT), sys.value());
      BOOST_REQUIRE_EQUAL("4,SEAT", sys.to_string());
      BOOST_REQUIRE_EQUAL("SEAT", sys.name());
   ...
   {
      BOOST_CHECK_EXCEPTION(symbol malformed(SY(6,SeaT)),
                            symbol_type_exception, fc_exception_message_is("invalid symbol: SeaT"));
   }

   // invalid - contains lower case characters, exception thrown
   {
      BOOST_CHECK_EXCEPTION(symbol(5,"SeaT"),
...
BOOST_FIXTURE_TEST_CASE( test_input_quantity, currency_tester ) try {
   ...
   {
      auto trace = transfer(cubetrain_tk, N(alice), "100.0000 CUR");
   ...
   {
      BOOST_REQUIRE_THROW(transfer(N(alice), N(carl), "20.50 USD"), cubetrain_assert_message_exception);
   }
