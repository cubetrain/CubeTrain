...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(cubetrain.tk), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create cubetrain.ms and cubetrain.tk
        create_accounts({N(cubetrain.ms), N(cubetrain.tk), N(cubetrain.rm), N(cubetrain.rf), N(cubetrain.st), N(cubetrain.vp), N(cubetrain.bp), N(cubetrain.sv) });

        // Set code for the following accounts:
        //  - cubetrain (code: cubetrain.bios) (already set by tester constructor)
        //  - cubetrain.ms (code: cubetrain.ms)
        //  - cubetrain.tk (code: cubetrain.tk)
        set_code_abi(N(cubetrain.ms), cubetrain_ms_wast, cubetrain_ms_abi);//, &cubetrain_active_pk);
        set_code_abi(N(cubetrain.tk), cubetrain_tk_wast, cubetrain_tk_abi); //, &cubetrain_active_pk);

        // Set privileged for cubetrain.ms and cubetrain.tk
        set_privileged(N(cubetrain.ms));
        set_privileged(N(cubetrain.tk));

        // Verify cubetrain.ms and cubetrain.tk is privileged
        const auto& cubetrain_ms_acc = get<account_object, by_name>(N(cubetrain.ms));
        BOOST_TEST(cubetrain_ms_acc.privileged == true);
        const auto& cubetrain_tk_acc = get<account_object, by_name>(N(cubetrain.tk));
        BOOST_TEST(cubetrain_tk_acc.privileged == true);


        // Create SEAT tokens in cubetrain.tk, set its manager as cubetrain
        ...
        create_currency(N(cubetrain.tk), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SEAT tokens to cubetrain.system
        issue(N(cubetrain.tk), config::system_account_name, config::system_account_name, initial_supply);
        ...
        // Set cubetrain.system to cubetrain
        set_code_abi(config::system_account_name, cubetrain_system_wast, cubetrain_system_abi);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(cubetrain.st), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "cubetrain");
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), cubetrain_assert_message_exception);
        ...
        const auto first_june_2018 = fc::seconds(1541865600); // 2018-11-11 00:00:00
        const auto first_june_2028 = fc::seconds(1857484800); // 2028-11-11 00:00:00
        // Ensure that now is yet 10 years after 2018-11-11 00:00:00 yet
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), cubetrain_assert_message_exception);
