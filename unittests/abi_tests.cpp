...
auto get_resolver(const abi_def& abi = abi_def())
{
   return [&abi](const account_name &name) -> optional<abi_serializer> {
      return abi_serializer(cubetrain_contract_abi(abi), max_serialization_time);
   };
}
...
{ try {
   ...
   abi_serializer abis(cubetrain_contract_abi(abi), max_serialization_time);
...
using namespace cubetrain::unittests::config;

struct abi_gen_helper {

  abi_gen_helper() {}

  static bool is_abi_generation_exception(const cubetrain::abi_generation_exception& e) { return true; };

  bool generate_abi(const char* source, const char* abi, bool opt_sfs=false) {

    std::string include_param = std::string("-I") + cubetrainlib_path;
    ...
    std::string stdcpp_include_param = std::string("-I") + cubetrainlib_path + "/libc++/upstream/include";
    std::string stdc_include_param = std::string("-I") + cubetrainlib_path +  "/musl/upstream/include";
    ...
    bool res = runToolOnCodeWithArgs(
      new find_cubetrain_abi_macro_action(contract, actions, ""),
      source,
      extra_args      
    );
    FC_ASSERT(res == true);
...
{ try {
   ...
   BOOST_CHECK_EXCEPTION( generate_abi(unknown_type, ""), cubetrain::abi_generation_exception, abi_gen_helper::is_abi_generation_exception );
...
{  try {
   ...
   #include <cubetrainlib/varint.hpp>
   #include <cubetrainlib/asset.hpp>
   #include <cubetrainlib/time.hpp>
...
{ try {
   ...
   BOOST_CHECK_EXCEPTION( generate_abi(double_base, ""), cubetrain::abi_generation_exception, abi_gen_helper::is_abi_generation_exception );
...
{ try {
   ...
   BOOST_CHECK_EXCEPTION( generate_abi(unable_to_determine_index, ""), cubetrain::abi_generation_exception, abi_gen_helper::is_abi_generation_exception );
...
{ try {
   ...
   BOOST_CHECK_EXCEPTION( generate_abi(same_type_different_namespace, ""), cubetrain::abi_generation_exception, abi_gen_helper::is_abi_generation_exception );
...
{ try {
   ...
   BOOST_CHECK_EXCEPTION( generate_abi(bad_index_type, "{}"), cubetrain::abi_generation_exception, abi_gen_helper::is_abi_generation_exception );
...
{ try {
   ...
   BOOST_CHECK_EXCEPTION( generate_abi(union_table, ""), cubetrain::abi_generation_exception, abi_gen_helper::is_abi_generation_exception );
...
{ try {
   ...
   BOOST_CHECK_EXCEPTION( generate_abi(same_action_different_type, ""), cubetrain::abi_generation_exception, abi_gen_helper::is_abi_generation_exception );
} FC_LOG_AND_RETHROW() }
...
{ try {
   ...
   BOOST_CHECK_EXCEPTION( generate_abi(abigen_vector_multidimension, ""), cubetrain::abi_generation_exception, abi_gen_helper::is_abi_generation_exception );
...
BOOST_FIXTURE_TEST_CASE(abigen_cubetrainabi_macro, abi_gen_helper)
{ try {

   const char* abigen_cubetrainabi_macro = R"=====(
      ...
      struct hello : public cubetrain::contract {
        public:
   ...
   const char* abigen_cubetrainabi_macro_abi = R"=====(
   {
     "types": [],
     "structs": [{
         "name": "hi",
   ...
   BOOST_TEST( generate_abi(abigen_cubetrainabi_macro, abigen_cubetrainabi_macro_abi) == true );
...
{ try {
   ...
   const char* abigen_contract_inheritance = R"=====(
      ...
      CUBETRAIN_ABI(new_hello,(hi)(bye))
...
BOOST_FIXTURE_TEST_CASE(abigen_no_cubetrainabi_macro, abi_gen_helper)
{ try {

   const char* abigen_no_cubetrainabi_macro = R"=====(
      #pragma GCC diagnostic push
      #pragma GCC diagnostic ignored "-Wpointer-bool-conversion"
      #include <cubetrainlib/cubetrain.hpp>
      #include <cubetrainlib/print.hpp>
   ...
   const char* abigen_no_cubetrainabi_macro_abi = R"=====(
   {
     "version": "cubetrain::abi/1.0",
     "types": [],
     "structs": [{
   ...
   BOOST_TEST( generate_abi(abigen_no_cubetrainabi_macro, abigen_no_cubetrainabi_macro_abi) == true );
...
{ try {
   ...
   auto abi = cubetrain_contract_abi(fc::json::from_string(my_abi).as<abi_def>());

   abi_serializer abis(abi, max_serialization_time);

   const char *my_other = R"=====(
    {
      "publickey"     :  "SEAT6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV",
      "publickey_arr" :  ["SEAT6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV","SEAT6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV","SEAT6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV"],
      "asset"         : "100.0000 SEAT",
      "asset_arr"     : ["100.0000 SEAT","100.0000 SEAT"],
      ...
      "keyweight": {"key":"SEAT6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":"100"},
      "keyweight_arr": [{"key":"SEAT6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":"100"},{"key":"SEAT6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":"200"}],
      "authority": {
         "threshold":"10",
         "keys":[{"key":"SEAT6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":100},{"key":"SEAT6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":200}],
...
{ try {
   ...
   auto abi = cubetrain_contract_abi(fc::json::from_string(typedef_cycle_abi).as<abi_def>());
...
{ try {
   ...
   abi_serializer abis(cubetrain_contract_abi(abi_def()), max_serialization_time);
...
{ try {
   ...
   const char* test_data = R"=====(
   {
     ...
     "auth" : {
        ...
        "keys" : [ {"key" : "SEAT65rXebLhtk2aTTzP4e9x1AQZs7c5NNXJp89W8R3HyaA6Zyd4im", "weight" : 57005},
                   {"key" : "SEAT5eVr9TVnqwnUBNwf9kwMTbrHvX5aPyyEG97dz2b2TNeqWRzbJf", "weight" : 57605} ],
   ...
   BOOST_TEST("SEAT65rXebLhtk2aTTzP4e9x1AQZs7c5NNXJp89W8R3HyaA6Zyd4im" == (std::string)updauth.auth.keys[0].key);
   BOOST_TEST(57005u == updauth.auth.keys[0].weight);
   BOOST_TEST("SEAT5eVr9TVnqwnUBNwf9kwMTbrHvX5aPyyEG97dz2b2TNeqWRzbJf" == (std::string)updauth.auth.keys[1].key);
...
{ try {
   ...
   BOOST_TEST("SEAT65rXebLhtk2aTTzP4e9x1AQZs7c5NNXJp89W8R3HyaA6Zyd4im" == (std::string)newacct.owner.keys[0].key);
   BOOST_TEST(57005u == newacct.owner.keys[0].weight);
   BOOST_TEST("SEAT5eVr9TVnqwnUBNwf9kwMTbrHvX5aPyyEG97dz2b2TNeqWRzbJf" == (std::string)newacct.owner.keys[1].key);
   ...
   BOOST_TEST("SEAT65rXebLhtk2aTTzP4e9x1AQZs7c5NNXJp89W8R3HyaA6Zyd4im" == (std::string)newacct.active.keys[0].key);
   BOOST_TEST(57005u == newacct.active.keys[0].weight);
   BOOST_TEST("SEAT5eVr9TVnqwnUBNwf9kwMTbrHvX5aPyyEG97dz2b2TNeqWRzbJf" == (std::string)newacct.active.keys[1].key);
...
{ try {
   ...
   abi_serializer abis(cubetrain_contract_abi(v.as<abi_def>()), max_serialization_time);
...
{ try {
   ...
   auto abi = cubetrain_contract_abi(fc::json::from_string(repeat_abi).as<abi_def>());
...
BOOST_AUTO_TEST_CASE(abi_account_name_in_cubetrain_abi)
{ try {
   // inifinite loop in types
   const char* repeat_abi = R"=====(
   {
...
BOOST_AUTO_TEST_CASE(abi_large_array)
{
   try {
      ...
      // curl http://127.0.0.1:8888/v1/chain/abi_bin_to_json -X POST -d '{"code":"cubetrain", "action":"hi", "binargs":"ffffffff08"}'
...
BOOST_AUTO_TEST_CASE(abi_recursive_structs)
{
   try {
      ...
      abi_serializer abis(fc::json::from_string(abi_str).as<abi_def>(), max_serialization_time);
      string hi_data = "{\"user\":\"cubetrain\",\"arg2\":{\"user\":\"1\"}}";
