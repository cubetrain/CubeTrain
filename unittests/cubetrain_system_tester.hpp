...
namespace cubetrain_system {

class cubetrain_system_tester : public TESTER {
public:

   cubetrain_system_tester()
   : cubetrain_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   cubetrain_system_tester(Lambda setup) {
      setup(*this);

      produce_blocks( 2 );

      create_accounts({ N(cubetrain.tk), N(cubetrain.rm), N(cubetrain.rf), N(cubetrain.st),
               N(cubetrain.bp), N(cubetrain.vp), N(cubetrain.sv), N(cubetrain.nm) });
      ...
      {
         const auto& accnt = control->db().get<account_object,by_name>( N(cubetrain.tk) );
         abi_def abi;
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance("cubetrain")  + get_balance("cubetrain.rf") + get_balance("cubetrain.st") + get_balance("cubetrain.rm") );
   }
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& seatin ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",seatin) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(cubetrain.tk), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
   ...
   fc::variant get_stats( const string& symbolname ) {
      auto symb = cubetrain::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(cubetrain.tk), symbol_code, N(stat), symbol_code );
   ...
   fc::variant get_global_state() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "cubetrain_global_state", data, abi_serializer_max_time );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(cubetrain.ms), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( "cubetrain", "cubetrain.ms", core_from_string("5000.0000") ) );
         produce_block();
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( "cubetrain", "alice1111111", core_from_string("650000000.0000"), "cubetrain" );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::cubetrain_code_name}, 1},
...
inline uint64_t M( const string& seat_str ) {
   return core_from_string( seat_str ).get_amount();
}
