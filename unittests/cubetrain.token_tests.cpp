...
class cubetrain_tk_tester : public tester {
public:

   cubetrain_tk_tester() {
      produce_blocks( 2 );

      create_accounts( { N(alice), N(bob), N(carol), N(cubetrain.tk) } );
      produce_blocks( 2 );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = cubetrain::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(cubetrain.tk), acc, N(accounts), symbol_code );
   ...
   action_result create( account_name issuer,
      ...
      return push_action( N(cubetrain.tk), N(create), mvo()
           ( "issuer", issuer)
...
BOOST_AUTO_TEST_SUITE(cubetrain_tk_tests)

BOOST_FIXTURE_TEST_CASE( create_tests, cubetrain_tk_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_negative_max_supply, cubetrain_tk_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( symbol_already_exists, cubetrain_tk_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_supply, cubetrain_tk_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_decimals, cubetrain_tk_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( issue_tests, cubetrain_tk_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( transfer_tests, cubetrain_tk_tester ) try {
