...
#include <cubetrain.ms/cubetrain.ms.wast.hpp>
#include <cubetrain.ms/cubetrain.ms.abi.hpp>
...
#include <cubetrain.system/cubetrain.system.wast.hpp>
#include <cubetrain.system/cubetrain.system.abi.hpp>
...
class cubetrain_ms_tester : public tester {
public:

   cubetrain_ms_tester() {
      create_accounts( { N(cubetrain.ms), N(cubetrain.st), N(cubetrain.rm), N(cubetrain.rf), N(alice), N(bob), N(carol) } );
      produce_block();

      auto trace = base_tester::push_action(config::system_account_name, N(setpriv),
                                            config::system_account_name,  mutable_variant_object()
                                            ("account", "cubetrain.ms")
                                            ("is_priv", 1)
      );

      set_code( N(cubetrain.ms), cubetrain_ms_wast );
      set_abi( N(cubetrain.ms), cubetrain_ms_abi );

      produce_blocks();
      const auto& accnt = control->db().get<account_object,by_name>( N(cubetrain.ms) );
      abi_def abi;
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(cubetrain.tk), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const string& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(cubetrain.tk), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
   ...
   asset get_balance( const account_name& act ) {
      ...
      const auto* tbl = db.find<table_id_object, by_code_scope_table>(boost::make_tuple(N(cubetrain.tk), act, N(accounts)));
      share_type result = 0;
   ...
   transaction_trace_ptr push_action( const account_name& signer, const action_name& name, const variant_object& data, bool auth = true ) {
      ...
      auto trace = base_tester::push_action( N(cubetrain.ms), name, accounts, data );
      produce_block();
...
transaction cubetrain_ms_tester::reqauth( account_name from, const vector<permission_level>& auths, const fc::microseconds& max_serialization_time ) {
   fc::variants v;
...
BOOST_AUTO_TEST_SUITE(cubetrain_ms_tests)

BOOST_FIXTURE_TEST_CASE( propose_approve_execute, cubetrain_ms_tester ) try {
   ...
   BOOST_REQUIRE_EXCEPTION( push_action( N(alice), N(exec), mvo()
                            ...
                            cubetrain_assert_message_is("transaction authorization failed")
   );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_unapprove, cubetrain_ms_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_by_two, cubetrain_ms_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( propose_with_wrong_requested_auth, cubetrain_ms_tester ) try {
   auto trx = reqauth("alice", vector<permission_level>{ { N(alice), config::active_name },  { N(bob), config::active_name } }, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( big_transaction, cubetrain_ms_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, cubetrain_ms_tester ) try {

   // required to set up the link between (cubetrain active) and (cubetrain.prods active)
   //
   //                  cubetrain active
   //                       |
   //             cubetrain.prods active (2/3 threshold)
   ...
   set_authority(config::system_account_name, "active", authority(1,
      vector<key_weight>{{get_private_key("cubetrain", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{N(cubetrain.prods), config::active_name}, 1}}), "owner",
      { { config::system_account_name, "active" } }, { get_private_key( config::system_account_name, "active" ) });

   set_producers( {N(alice),N(bob),N(carol)} );
   produce_blocks(50);

   create_accounts( { N(cubetrain.tk) } );
   set_code( N(cubetrain.tk), cubetrain_tk_wast );
   set_abi( N(cubetrain.tk), cubetrain_tk_abi );

   create_currency( N(cubetrain.tk), config::system_account_name, core_from_string("10000000000.0000") );
   issue(config::system_account_name, core_from_string("1000000000.0000"));
   BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"),
                        get_balance("cubetrain") + get_balance("cubetrain.rf") + get_balance("cubetrain.st") + get_balance("cubetrain.rm") );

   set_code( config::system_account_name, cubetrain_system_wast );
   set_abi( config::system_account_name, cubetrain_system_abi );
   ...
   // execute by alice to replace the cubetrain system contract
   transaction_trace_ptr trace;
   control->applied_transaction.connect([&]( const transaction_trace_ptr& t) { if (t->scheduled) { trace = t; } } );
   ...
   BOOST_REQUIRE_EXCEPTION( create_account_with_resources( N(alice1111112), config::system_account_name, core_from_string("1.0000"), false ),
                            cubetrain_assert_message_exception, cubetrain_assert_message_is("Unknown Test")

   );
} FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE( update_system_contract_major_approve, cubetrain_ms_tester ) try {

   // set up the link between (cubetrain active) and (cubetrain.prods active)
   set_authority(config::system_account_name, "active", authority(1,
      vector<key_weight>{{get_private_key("cubetrain", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{N(cubetrain.prods), config::active_name}, 1}}), "owner",
      { { config::system_account_name, "active" } }, { get_private_key( config::system_account_name, "active" ) });
   ...
   create_currency( N(cubetrain.tk), config::system_account_name, core_from_string("10000000000.0000") );
   issue(config::system_account_name, core_from_string("1000000000.0000"));
   BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( "cubetrain" ) );
   ...
   BOOST_REQUIRE_EXCEPTION(
      ...
      cubetrain_assert_message_exception, cubetrain_assert_message_is("transaction authorization failed")
   );
