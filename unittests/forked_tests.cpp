...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(cubetrain.tk)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(cubetrain.tk), cubetrain_tk_wast );
   c.set_abi( N(cubetrain.tk), cubetrain_tk_abi );
   c.produce_blocks(10);


   auto cr = c.push_action( N(cubetrain.tk), N(create), N(cubetrain.tk), mutable_variant_object()
              ("issuer",       "cubetrain" )
   ...
   cr = c.push_action( N(cubetrain.tk), N(issue), config::system_account_name, mutable_variant_object()
              ("to",       "dan" )
