...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for cubetrain reqauth action with alice's spending key
   chain.link_authority("alice", "cubetrain", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "cubetrain", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with cubetrain reqauth
   chain.unlink_authority("alice", "cubetrain", "reqauth");
   ...
   // Link authority for any cubetrain action with alice's scud key
   chain.link_authority("alice", "cubetrain", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "cubetrain", "first",  "reqauth");
...
try {
   ...
   // Creating account with cubetrain. prefix with privileged account
   chain.create_account("cubetrain.test1");

   // Creating account with cubetrain. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("cubetrain.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'cubetrain.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "cubetrain", "cubetrain.any", "reqauth" );
   chain.link_authority( "bob", "cubetrain", "cubetrain.any", "reqauth" );

   /// this should succeed because cubetrain::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = cubetrain::chain::resource_limits::resource_usage_object;
   using by_owner = cubetrain::chain::resource_limits::by_owner;
...
BOOST_AUTO_TEST_CASE( linkauth_special ) { try {
   ...
   auto validate_disallow = [&] (const char *type) {
      BOOST_REQUIRE_EXCEPTION(
         ...
         fc_exception_message_is(std::string("Cannot link cubetrain::") + std::string(type) + std::string(" to a minimum permission"))
      );
   };
