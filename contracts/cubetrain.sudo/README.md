...
Execute a transaction while bypassing regular authorization checks (requires authorization of cubetrain.sudo which needs to be a privileged account).

### cubetrain.sudo::exec    executer trx
...
## 2. Installing the cubetrain.sudo contract

The cubetrain.sudo contract needs to be installed on a privileged account to function. It is recommended to use the account `cubetrain.sudo`.

First, the account `cubetrain.sudo` needs to be created. Since it has the restricted `cubetrain.` prefix, only a privileged account can create this account. So this guide will use the `cubetrain` account to create the `cubetrain.sudo` account. On typical live blockchain configurations, the `cubetrain` account can only be controlled by a supermajority of the current active block producers. So, this guide will use the `cubetrain.ms` contract to help coordinate the approvals of the proposed transaction that creates the `cubetrain.sudo` account.

The `cubetrain.sudo` account also needs to have sufficient RAM to host the contract and sufficient CPU and network bandwidth to deploy the contract. This means that the creator of the account (`cubetrain`) needs to gift sufficient RAM to the new account and delegate (preferably with transfer) sufficient bandwidth to the new account. To pull this off the `cubetrain` account needs to have enough of the core system token (the `SEAT` token will be used within this guide) in its liquid balance. So prior to continuing with the next steps of this guide, the active block producers of the chain who are coordinating this process need to ensure that a sufficient amount of core system tokens that they are authorized to spend is placed in the liquid balance of the `cubetrain` account.

This guide will be using cliseat to carry out the process.

### 2.1 Create the cubetrain.sudo account

#### 2.1.1 Generate the transaction to create the cubetrain.sudo account

The transaction to create the `cubetrain.sudo` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the cliseat command to propose the transaction to the cubetrain.ms contract.

A simple way to generate a transaction to create a new account is to use the `cliseat system newaccount`. However, that sub-command currently only accepts a single public key as the owner and active authority of the new account. However, the owner and active authorities of the new account should only be satisfied by the `active` permission of `cubetrain`. One option is to create the new account with the some newly generated key, and then later update the authorities of the new account using `cliseat set account permission`. This guide will take an alternative approach which atomically creates the new account in its proper configuration.

Three unsigned transactions will be generated using cliseat and then the actions within those transactions will be appropriately stitched together into a single transaction which will later be proposed using the cubetrain.ms contract.

First, generate a transaction to capture the necessary actions involved in creating a new account:
```
$ cliseat system newaccount -s -j -d --transfer --stake-net "1.000 SEAT" --stake-cpu "1.000 SEAT" --buy-ram-kbytes 50 cubetrain cubetrain.sudo SEAT8MMUW11TAdTDxqdSwSqJodefSoZbFhcprndomgLi9MeR2o8MT4 > generated_account_creation_trx.json
726964ms thread-0   main.cpp:429                  create_action        ] result: {"binargs":"0000000000ea305500004d1a03ea305500c80000"} arg: {"code":"cubetrain","action":"buyrambytes","args":{"payer":"cubetrain","receiver":"cubetrain.sudo","bytes":51200}}
726967ms thread-0   main.cpp:429                  create_action        ] result: {"binargs":"0000000000ea305500004d1a03ea3055102700000000000004535953000000001027000000000000045359530000000001"} arg: {"code":"cubetrain","action":"delegatebw","args":{"from":"cubetrain","receiver":"cubetrain.sudo","stake_net_quantity":"1.0000 SEAT","stake_cpu_quantity":"1.0000 SEAT","transfer":true}}
...
The actual public key used is not important since that data is only encoded into the `cubetrain::newaccount` action which will be replaced soon anyway.

Second, create a file (e.g. newaccount_payload.json) with the JSON payload for the real `cubetrain::newaccount` action. It should look like:
```
$ cat newaccount_payload.json
{
   ...
   "owner": {
      ...
      "keys": [],
      "accounts": [{
         "permission": {"actor": "cubetrain", "permission": "active"},
         "weight": 1
      }],
      "waits": []
   },
...
Third, generate a transaction containing the actual `cubetrain::newaccount` action that will be used in the final transaction:
```
$ cliseat push action -s -j -d cubetrain newaccount newaccount_payload.json -p cubetrain > generated_newaccount_trx.json
...
Fourth, generate a transaction containing the `cubetrain::setpriv` action which will make the `cubetrain.sudo` account privileged:
```
$ cliseat push action -s -j -d cubetrain setpriv '{"account": "cubetrain.sudo", "is_priv": 1}' -p cubetrain > generated_setpriv_trx.json
...
Next, the action JSONs of the previously generated transactions will be used to construct a unified transaction which will eventually be proposed with the cubetrain.ms contract. A good way to get started is to make a copy of the generated_newaccount_trx.json file (call the copied file create_sudo_account_trx.json) and edit the first three fields so it looks something like the following:
```
...
It will be useful to have a JSON of the active permissions of each of the active block producers for later when proposing transactions using the cubetrain.ms contract.
...
#### 2.1.2 Propose the transaction to create the cubetrain.sudo account
...
```
$ cliseat multisig propose_trx createsudo producer_permissions.json create_sudo_account_trx.json blkproducera
executed transaction: bf6aaa06b40e2a35491525cb11431efd2b5ac94e4a7a9c693c5bf0cfed942393  744 bytes  772 us
#    cubetrain.ms <= cubetrain.ms::propose          {"proposer":"blkproducera","proposal_name":"createsudo","requested":[{"actor":"blkproducera","permis...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.1.3 Review and approve the transaction to create the cubetrain.sudo account
...
The proposed transaction can be reviewed using the `cliseat multisig review` command:
```
$ cliseat multisig review blkproducera createsudo > create_sudo_account_trx_to_review.json
...
```
$ cliseat multisig propose_trx -j -s -d createsudo '[]' create_sudo_account_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_create_sudo_trx_serialized.hex
...
```
$ cliseat multisig approve blkproducera createsudo '{"actor": "blkproducerb", "permission": "active"}' -p blkproducerb
executed transaction: 03a907e2a3192aac0cd040c73db8273c9da7696dc7960de22b1a479ae5ee9f23  128 bytes  472 us
#    cubetrain.ms <= cubetrain.ms::approve          {"proposer":"blkproducera","proposal_name":"createsudo","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.1.4 Execute the transaction to create the cubetrain.sudo account

When the necessary approvals are collected (in this example, with 21 block producers, at least 15 of their approvals were required), anyone can push the `cubetrain.ms::exec` action which executes the approved transaction. It makes a lot of sense for the lead block producer who proposed the transaction to also execute it (this will incur another temporary RAM cost for the deferred transaction that is generated by the cubetrain.ms contract).

```
$ cliseat multisig exec blkproducera createsudo blkproducera
executed transaction: 7ecc183b99915cc411f96dde7c35c3fe0df6e732507f272af3a039b706482e5a  160 bytes  850 us
#    cubetrain.ms <= cubetrain.ms::exec             {"proposer":"blkproducera","proposal_name":"createsudo","executer":"blkproducera"}
warning: transaction executed locally, but may not be confirmed by the network yet
```

Anyone can now verify that the `cubetrain.sudo` was created:
```
$ cliseat get account cubetrain.sudo
privileged: true
permissions:
     owner     1:    1 cubetrain@active,
        active     1:    1 cubetrain@active,
memory:
     quota:     49.74 KiB    used:     3.33 KiB  

net bandwidth:
     staked:          1.0000 SEAT           (total stake delegated from account to self)
     delegated:       0.0000 SEAT           (total staked delegated to account from others)
     used:                 0 bytes
     available:        2.304 MiB  
     limit:            2.304 MiB  
...
### 2.2 Deploy the cubetrain.sudo contract

#### 2.2.1  Generate the transaction to deploy the cubetrain.sudo contract

The transaction to deploy the contract to the `cubetrain.sudo` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the cliseat command to propose the transaction to the cubetrain.ms contract.

The easy way to generate this transaction is using cliseat:
```
$ cliseat set contract -s -j -d cubetrain.sudo contracts/cubetrain.sudo/ > deploy_sudo_contract_trx.json
Reading WAST/WASM from contracts/cubetrain.sudo/cubetrain.sudo.wasm...
...
#### 2.2.2 Propose the transaction to deploy the cubetrain.sudo contract
...
```
$ cliseat multisig propose_trx deploysudo producer_permissions.json deploy_sudo_contract_trx.json blkproducera
executed transaction: 9e50dd40eba25583a657ee8114986a921d413b917002c8fb2d02e2d670f720a8  4312 bytes  871 us
#    cubetrain.ms <= cubetrain.ms::propose          {"proposer":"blkproducera","proposal_name":"deploysudo","requested":[{"actor":"blkproducera","permis...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.2.3 Review and approve the transaction to deploy the cubetrain.sudo contract

Each of the potential approvers of the proposed transaction (i.e. the active block producers) should first review the proposed transaction to make sure they are not approving anything that they do not agree to.

The proposed transaction can be reviewed using the `cliseat multisig review` command:
```
$ cliseat multisig review blkproducera deploysudo > deploy_sudo_contract_trx_to_review.json
...
```
$ cliseat multisig propose_trx -j -s -d deploysudo '[]' deploy_sudo_contract_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_deploy_sudo_trx_serialized.hex
...
When an approver (e.g. `blkproducerb`) is satisfied with the proposed transaction, they can simply approve it:
```
$ cliseat multisig approve blkproducera deploysudo '{"actor": "blkproducerb", "permission": "active"}' -p blkproducerb
executed transaction: d1e424e05ee4d96eb079fcd5190dd0bf35eca8c27dd7231b59df8e464881abfd  128 bytes  483 us
#    cubetrain.ms <= cubetrain.ms::approve          {"proposer":"blkproducera","proposal_name":"deploysudo","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.2.4 Execute the transaction to create the cubetrain.sudo account

When the necessary approvals are collected (in this example, with 21 block producers, at least 15 of their approvals were required), anyone can push the `cubetrain.ms::exec` action which executes the approved transaction. It makes a lot of sense for the lead block producer who proposed the transaction to also execute it (this will incur another temporary RAM cost for the deferred transaction that is generated by the cubetrain.ms contract).

```
$ cliseat multisig exec blkproducera deploysudo blkproducera
executed transaction: e8da14c6f1fdc3255b5413adccfd0d89b18f832a4cc18c4324ea2beec6abd483  160 bytes  1877 us
#    cubetrain.ms <= cubetrain.ms::exec             {"proposer":"blkproducera","proposal_name":"deploysudo","executer":"blkproducera"}
```

Anyone can now verify that the `cubetrain.sudo` contract was deployed correctly.

```
$ cliseat get code -a retrieved-cubetrain.sudo.abi cubetrain.sudo
code hash: 1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c
saving abi to retrieved-cubetrain.sudo.abi
$ sha256sum contracts/cubetrain.sudo/cubetrain.sudo.wasm
1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c  contracts/cubetrain.sudo/cubetrain.sudo.wasm
```

If the two hashes match then the local WebAssembly code is the one deployed on the blockchain. The retrieved ABI, which was stored in the file retrieved-cubetrain.sudo.abi, can then be compared to the original ABI of the contract (contracts/cubetrain.sudo/cubetrain.sudo.abi) to ensure they are semantically the same.

## 3. Using the cubetrain.sudo contract

### 3.1 Example: Updating owner authority of an arbitrary account

This example will demonstrate how to use the deployed cubetrain.sudo contract together with the cubetrain.ms contract to allow a greater than two-thirds supermajority of block producers of an CUBETRAIN blockchain to change the owner authority of an arbitrary account. The example will use cliseat: in particular, the `cliseat multisig` command, the `cliseat set account permission` sub-command, and the `cliseat sudo exec` sub-command. However, the guide also demonstrates what to do if the `cliseat sudo exec` sub-command is not available.
...
```
permissions:
     owner     1:    1 SEAT6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV
        active     1:    1 SEAT6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV
memory:
...
Assume that none of the block producers know the private key corresponding to the public key `SEAT6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV` which, as can be seen above, is initially securing access to the `alice` account.
...
```
$ cliseat set account permission -s -j -d alice owner '{"threshold": 1, "accounts": [{"permission": {"actor": "cubetrain", "permission": "active"}, "weight": 1}]}' > update_alice_owner_trx.json
```
...
The next step is to generate the transaction containing the `cubetrain.sudo::exec` action. This action will contain the transaction in update_alice_owner_trx.json as part of its action payload data.

```
$ cliseat sudo exec -s -j -d blkproducera update_alice_owner_trx.json > sudo_update_alice_owner_trx.json
```
...
If the `cliseat sudo` command is not available, there is an alternative way to generate the above transaction. There is no need to continue reading the remaining of sub-section 3.1.1 if the sudo_update_alice_owner_trx.json file was already generated with content similar to the above using the `cliseat sudo exec` sub-command method.
...
```
$ cliseat multisig propose_trx -s -j -d nothing '[]' update_alice_owner_trx.json nothing | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > update_alice_owner_trx_serialized.hex
...
Then generate the template for the transaction containing the `cubetrain.sudo::exec` action:
```
$ cliseat push action -s -j -d cubetrain.sudo exec '{"executer": "blkproducera", "trx": ""}' > sudo_update_alice_owner_trx.json
$ cat sudo_update_alice_owner_trx.json
{
...
```
$ cliseat multisig propose_trx updatealice producer_permissions.json sudo_update_alice_owner_trx.json blkproducera
executed transaction: 10474f52c9e3fc8e729469a577cd2fc9e4330e25b3fd402fc738ddde26605c13  624 bytes  782 us
#    cubetrain.ms <= cubetrain.ms::propose          {"proposer":"blkproducera","proposal_name":"updatealice","requested":[{"actor":"blkproducera","permi...
warning: transaction executed locally, but may not be confirmed by the network yet
```
...
Each of the potential approvers of the proposed transaction (i.e. the active block producers) should first review the proposed transaction to make sure they are not approving anything that they do not agree to.
```
$ cliseat multisig review blkproducera updatealice > sudo_update_alice_owner_trx_to_review.json
...
The approvers should go through the human-readable transaction output and make sure everything looks fine. However, due to a current limitation of nodeseat/cliseat, the JSONification of action payload data does not occur recursively. So while both the `hex_data` and the human-readable JSON `data` of the payload of the `cubetrain.sudo::exec` action is available in the output of the `cliseat multisig review` command, only the hex data is available of the payload of the inner `cubetrain::updateauth` action. So it is not clear what the `updateauth` will actually do.

Furthermore, even if this usability issue was fixed in nodeseat/cliseat, there will still be cases where there is no sensible human-readable version of an action data payload within a transaction. An example of this is the proposed transaction in sub-section 2.2.3 which used the `cubetrain::setcode` action to set the contract code of the `cubetrain.sudo` account. The best thing to do in such situations is for the reviewer to compare the proposed transaction to one generated by them through a process they trust.
...
```
$ cliseat multisig propose_trx -j -s -d updatealice '[]' sudo_update_alice_owner_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_sudo_update_alice_owner_trx_serialized.hex
...
When an approver (e.g. `blkproducerb`) is satisfied with the proposed transaction, they can simply approve it:
```
$ cliseat multisig approve blkproducera updatealice '{"actor": "blkproducerb", "permission": "active"}' -p blkproducerb
executed transaction: 2bddc7747e0660ba26babf95035225895b134bfb2ede32ba0a2bb6091c7dab56  128 bytes  543 us
#    cubetrain.ms <= cubetrain.ms::approve          {"proposer":"blkproducera","proposal_name":"updatealice","level":{"actor":"blkproducerb","permission...
warning: transaction executed locally, but may not be confirmed by the network yet
```
...
```
$ cliseat multisig exec blkproducera updatealice blkproducera
executed transaction: 7127a66ae307fbef6415bf60c3e91a88b79bcb46030da983c683deb2a1a8e0d0  160 bytes  820 us
#    cubetrain.ms <= cubetrain.ms::exec             {"proposer":"blkproducera","proposal_name":"updatealice","executer":"blkproducera"}
warning: transaction executed locally, but may not be confirmed by the network yet
```

Anyone can now verify that the owner authority of `alice` was successfully changed:
```
$ cliseat get account alice
permissions:
     owner     1:    1 cubetrain@active,
        active     1:    1 SEAT6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV
memory:
