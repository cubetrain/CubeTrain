#include <cubetrain.ms/cubetrain.ms.hpp>
#include <cubetrainlib/action.hpp>
#include <cubetrainlib/permission.hpp>
...
void multisig::propose() {
   ...
   cubetrain_assert( trx_header.expiration >= cubetrain::time_point_sec(now()), "transaction expired" );
   //cubetrain_assert( trx_header.actions.size() > 0, "transaction must have at least one action" );

   proposals proptable( _self, proposer );
   cubetrain_assert( proptable.find( proposal_name ) == proptable.end(), "proposal with the same name exists" );
   ...
   cubetrain_assert( res > 0, "transaction authorization failed" );
...
void multisig::approve( account_name proposer, name proposal_name, permission_level level ) {
   ...
   cubetrain_assert( itr != apps.requested_approvals.end(), "approval is not on the list of requested approvals" );
...
void multisig::unapprove( account_name proposer, name proposal_name, permission_level level ) {
   ...
   cubetrain_assert( itr != apps.provided_approvals.end(), "no approval previously granted" );
...
void multisig::cancel( account_name proposer, name proposal_name, account_name canceler ) {
   ...
   if( canceler != proposer ) {
      cubetrain_assert( unpack<transaction_header>( prop.packed_transaction ).expiration < cubetrain::time_point_sec(now()), "cannot cancel until expiration" );
   }
...
CUBETRAIN_ABI( cubetrain::multisig, (propose)(approve)(unapprove)(cancel)(exec) )
