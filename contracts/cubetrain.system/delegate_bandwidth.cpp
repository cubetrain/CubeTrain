...
namespace cubetrainsystem {
   ...
   using cubetrain::permission_level;
   using std::map;
   using std::pair;
   ...
   struct user_resources {
      ...
      SEATLIB_SERIALIZE( user_resources, (owner)(net_weight)(cpu_weight)(ram_bytes) )
   };
   ...
   struct delegated_bandwidth {
      ...
      // explicit serialization macro is not necessary, used here only to improve compilation time
      SEATLIB_SERIALIZE( delegated_bandwidth, (from)(to)(net_weight)(cpu_weight) )
   ...
   struct refund_request {
      account_name  owner;
      time          request_time;
      cubetrain::asset  net_amount;
      cubetrain::asset  cpu_amount;

      uint64_t  primary_key()const { return owner; }

      // explicit serialization macro is not necessary, used here only to improve compilation time
      SEATLIB_SERIALIZE( refund_request, (owner)(request_time)(net_amount)(cpu_amount) )
   };
    ...
    */
   typedef cubetrain::multi_index< N(userres), user_resources>      user_resources_table;
   typedef cubetrain::multi_index< N(delband), delegated_bandwidth> del_bandwidth_table;
   typedef cubetrain::multi_index< N(refunds), refund_request>      refunds_table;
   ...
   void system_contract::buyrambytes( account_name payer, account_name receiver, uint32_t bytes ) {
      ...
      auto tmp = *itr;
      auto seatout = tmp.convert( asset(bytes,S(0,RAM)), CORE_SYMBOL );

      buyram( payer, receiver, seatout );
   }
   ...
   {
      require_auth( payer );
      cubetrain_assert( quant.amount > 0, "must purchase a positive amount" );
      ...
      INLINE_ACTION_SENDER(cubetrain::token, transfer)( N(cubetrain.tk), {payer,N(active)},
         { payer, N(cubetrain.rm), quant_after_fee, std::string("buy ram") } );

      if( fee.amount > 0 ) {
         INLINE_ACTION_SENDER(cubetrain::token, transfer)( N(cubetrain.tk), {payer,N(active)},
                                                       { payer, N(cubetrain.rf), fee, std::string("ram fee") } );
      }
      ...
      cubetrain_assert( bytes_out > 0, "must reserve a positive amount" );
   ...
   void system_contract::sellram( account_name account, int64_t bytes ) {
      ...
      cubetrain_assert( bytes > 0, "cannot sell negative byte" );

      user_resources_table  userres( _self, account );
      auto res_itr = userres.find( account );
      cubetrain_assert( res_itr != userres.end(), "no resource row" );
      cubetrain_assert( res_itr->ram_bytes >= bytes, "insufficient quota" );
      ...
      cubetrain_assert( tokens_out.amount > 1, "token amount received from selling ram is too low" );
      ...
      cubetrain_assert( _gstate.total_ram_stake >= 0, "error, attempt to unstake more tokens than previously staked" );
      ...
      INLINE_ACTION_SENDER(cubetrain::token, transfer)( N(cubetrain.tk), {N(cubetrain.rm),N(active)},
                                                       { N(cubetrain.rm), account, asset(tokens_out), std::string("sell ram") } );

      auto fee = ( tokens_out.amount + 199 ) / 200; /// .5% fee (round up)
      // since tokens_out.amount was asserted to be at least 2 earlier, fee.amount < tokens_out.amount
      
      if( fee > 0 ) {
         INLINE_ACTION_SENDER(cubetrain::token, transfer)( N(cubetrain.tk), {account,N(active)},
            { account, N(cubetrain.rf), asset(fee), std::string("sell ram fee") } );
      }
   }
   ...
   void validate_b1_vesting( int64_t stake ) {
      ...
      cubetrain_assert( max_claimable - claimable <= stake, "b1 can only claim their tokens over 10 years" );
   }
   ...
   {
      require_auth( from );
      cubetrain_assert( stake_net_delta != asset(0) || stake_cpu_delta != asset(0), "should stake non-zero amount" );
      cubetrain_assert( std::abs( (stake_net_delta + stake_cpu_delta).amount )
      ...
      // update stake delegated from "from" to "receiver"
      {
         ...
         cubetrain_assert( asset(0) <= itr->net_weight, "insufficient staked net bandwidth" );
         cubetrain_assert( asset(0) <= itr->cpu_weight, "insufficient staked cpu bandwidth" );
      ...
      // update totals of "receiver"
      {
         ...
         cubetrain_assert( asset(0) <= tot_itr->net_weight, "insufficient staked total net bandwidth" );
         cubetrain_assert( asset(0) <= tot_itr->cpu_weight, "insufficient staked total cpu bandwidth" );
      ...
      if ( N(cubetrain.st) != source_stake_from ) { //for cubetrain both transfer and refund make no sense
         ...
         if( is_delegating_to_self || is_undelegating ) {
            if ( req != refunds_tbl.end() ) { //need to update refund
               ...
               cubetrain_assert( asset(0) <= req->net_amount, "negative net refund amount" ); //should never happen
               cubetrain_assert( asset(0) <= req->cpu_amount, "negative cpu refund amount" ); //should never happen
         ...
         if ( asset(0) < transfer_amount ) {
            INLINE_ACTION_SENDER(cubetrain::token, transfer)( N(cubetrain.tk), {source_stake_from, N(active)},
               { source_stake_from, N(cubetrain.st), asset(transfer_amount), std::string("stake bandwidth") } );
         }
      }
      ...
      // update voting power
      {
         ...
         cubetrain_assert( 0 <= from_voter->staked, "stake for voting cannot be negative");
         if( from == N(b1) ) {
   ...
   {
      cubetrain_assert( stake_cpu_quantity >= asset(0), "must stake a positive amount" );
      cubetrain_assert( stake_net_quantity >= asset(0), "must stake a positive amount" );
      cubetrain_assert( stake_net_quantity + stake_cpu_quantity > asset(0), "must stake a positive amount" );
      cubetrain_assert( !transfer || from != receiver, "cannot use transfer flag if delegating to self" );
   ...
   {
      cubetrain_assert( asset() <= unstake_cpu_quantity, "must unstake a positive amount" );
      cubetrain_assert( asset() <= unstake_net_quantity, "must unstake a positive amount" );
      cubetrain_assert( asset() < unstake_cpu_quantity + unstake_net_quantity, "must unstake a positive amount" );
      cubetrain_assert( _gstate.total_activated_stake >= min_activated_stake,
   ...
   void system_contract::refund( const account_name owner ) {
      ...
      cubetrain_assert( req != refunds_tbl.end(), "refund request not found" );
      cubetrain_assert( req->request_time + refund_delay <= now(), "refund is not available yet" );
      // Until now() becomes NOW, the fact that now() is the timestamp of the previous block could in theory
      // allow people to get their tokens earlier than the 3 day delay if the unstake happened immediately after many
      // consecutive missed blocks.

      INLINE_ACTION_SENDER(cubetrain::token, transfer)( N(cubetrain.tk), {N(cubetrain.st),N(active)},
                                                    { N(cubetrain.st), req->owner, req->net_amount + req->cpu_amount, std::string("unstake") } );
