...
namespace cubetrain {
    ...
    * ```
    *
    * For this to work the Actions must be derived from cubetrain::contract
    *
    */
   ...
   bool dispatch( uint64_t code, uint64_t act ) {
      ...
      return cubetrain::dispatch<Contract,SecondAction,Actions...>( code, act );
   }
    ...
    * @tparam T - The contract class that has the correponding action handler, this contract should be derived from cubetrain::contract
...
// Helper macro for CUBETRAIN_API
#define CUBETRAIN_API_CALL( r, OP, elem ) \
   case ::cubetrain::string_to_name( BOOST_PP_STRINGIZE(elem) ): \
      cubetrain::execute_action( &thiscontract, &OP::elem ); \
      break;

// Helper macro for CUBETRAIN_ABI
#define CUBETRAIN_API( TYPE,  MEMBERS ) \
   BOOST_PP_SEQ_FOR_EACH( CUBETRAIN_API_CALL, TYPE, MEMBERS )
 ...
 * To be able to use this macro, the contract needs to be derived from cubetrain::contract
 * 
 ...
 * Example:
 * @code
 * CUBETRAIN_ABI( cubetrain::bios, (setpriv)(setalimits)(setglimits)(setprods)(reqauth) )
 * @endcode
 */
#define CUBETRAIN_ABI( TYPE, MEMBERS ) \
extern "C" { \
   void apply( uint64_t receiver, uint64_t code, uint64_t action ) { \
      auto self = receiver; \
      if( action == N(onerror)) { \
         /* onerror is only valid if it is for the "cubetrain" code account and authorized by "cubetrain"'s "active permission */ \
         cubetrain_assert(code == N(cubetrain), "onerror action's are only valid from the \"cubetrain\" system account"); \
      } \
      if( code == self || action == N(onerror) ) { \
         TYPE thiscontract( self ); \
         switch( action ) { \
            CUBETRAIN_API( TYPE, MEMBERS ) \
         } \
         /* does not allow destructor of thiscontract to run: cubetrain_exit(0); */ \
      } \
   } \
} \
 /// @}  dispatcher
