...
namespace cubetrain {
   ...
   void set_blockchain_parameters(const cubetrain::blockchain_parameters& params) {
      char buf[sizeof(cubetrain::blockchain_parameters)];
      cubetrain::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(cubetrain::blockchain_parameters& params) {
      char buf[sizeof(cubetrain::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      cubetrain_assert( size <= sizeof(buf), "buffer is too small" );
      cubetrain::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   private:
      ...
      public:
         ...
         {
            cubetrain_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return cubetrain::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = cubetrain::memory_heap.malloc(count*size);
...
{
   return cubetrain::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return cubetrain::memory_heap.free(ptr);
}
