...
namespace cubetrain {
   ...
   struct margin_position {
      ...
      SEATLIB_SERIALIZE( margin_position, (owner)(borrowed)(collateral)(call_price) )
   };

   typedef cubetrain::multi_index<N(margins), margin_position,
           indexed_by<N(callprice), cubetrain::const_mem_fun<margin_position, uint64_t, &margin_position::get_call> >
   > margins;
   ...
   struct loan_position {
      ...
      SEATLIB_SERIALIZE( loan_position, (owner)(interest_shares) )
   };

   typedef cubetrain::multi_index<N(loans), loan_position> loans;
