...
namespace cubetrain {
   ...
   struct margin_state {
      ...
      extended_asset unlend( double ishares ) {
         ...
         cubetrain_assert( interest_shares >= 0, "underflow" );
         cubetrain_assert( total_lendable.amount >= 0, "underflow" );

         return result;
      }

      SEATLIB_SERIALIZE( margin_state, (total_lendable)(total_lent)(least_collateralized)(interest_shares) )
   };
   ...
   struct exchange_state {
      ...
      struct connector {
         ...
         SEATLIB_SERIALIZE( connector, (balance)(weight)(peer_margin) )
      };
      ...
      SEATLIB_SERIALIZE( exchange_state, (manager)(supply)(fee)(base)(quote) )
   };

   typedef cubetrain::multi_index<N(markets), exchange_state> markets;
