...
void cubetrain_assert( bool test, const string& msg ) {
...
exchange_state convert( const exchange_state& current,
  ...
  cubetrain_assert( min_output.symbol != input.symbol, "cannot convert" );
  ...
  if( input.symbol != exchange_symbol ) {
     ...
     else cubetrain_assert( false, "invalid symbol" );
  } else {
