...
class simpletoken : public cubetrain::contract {
   public:
      ...
      void transfer( account_name from, account_name to, uint64_t quantity ) {
         ...
         cubetrain_assert( fromacnt.balance >= quantity, "overdrawn balance" );
   ...
   private:
      ...
      cubetrain::multi_index<N(accounts), account> _accounts;
      ...
      void add_balance( account_name payer, account_name to, uint64_t q ) {
         ...
         if( toitr == _accounts.end() ) {
           ...
           _accounts.modify( toitr, 0, [&]( auto& a ) {
              a.balance += q;
              cubetrain_assert( a.balance >= q, "overflow detected" );
           });
         }
      }
};

CUBETRAIN_ABI( simpletoken, (transfer)(issue) )
