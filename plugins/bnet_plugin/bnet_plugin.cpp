...
#include <cubetrain/bnet_plugin/bnet_plugin.hpp>
...
using cubetrain::public_key_type;
using cubetrain::chain_id_type;
using cubetrain::block_id_type;
using cubetrain::block_timestamp_type;
using std::string;
using cubetrain::sha256;
using cubetrain::signed_block_ptr;
using cubetrain::packed_transaction_ptr;
using std::vector;
...
namespace cubetrain {
       ...
       //    shared_ptr< vector<char> >  block_msg; ///< packed bnet_message for this block
        ...
        void on( const signed_block_ptr& b ) {
           ...
           if (!b) {
              ...
              SEAT_THROW(block_validate_exception, "bad block" );
           }
        ...
        void on( const packed_transaction_ptr& p ) {
           ...
           if (!p) {
              ...
              SEAT_THROW(transaction_exception, "bad transaction");
           }
  ...
  class listener : public std::enable_shared_from_this<listener> {
     ...
     public:
        ...
        void run() {
           SEAT_ASSERT( _acceptor.is_open(), plugin_exception, "unable top open listen socket" );
           do_accept();
        }
   ...
   void bnet_plugin::plugin_shutdown() {
      ...
      my->for_each_session([](auto ses){
         SEAT_ASSERT( false, plugin_exception, "session ${ses} still active", ("ses", ses->_session_num) );
      });
