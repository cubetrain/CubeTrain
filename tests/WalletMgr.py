...
class WalletMgr(object):
    __walletLogFile="test_keyseatd_output.log"
    __walletDataDir="test_wallet_0"

    # pylint: disable=too-many-arguments
    # walletd [True|False] True=Launch wallet(keyseatd) process; False=Manage launch process externally.
    def __init__(self, walletd, nodeseatPort=8888, nodeseatHost="localhost", port=9899, host="localhost"):
        self.walletd=walletd
        self.nodeseatPort=nodeseatPort
        self.nodeseatHost=nodeseatHost
        self.port=port
        self.host=host
        self.wallets={}
        self.__walletPid=None
        self.endpointArgs="--url http://%s:%d" % (self.nodeseatHost, self.nodeseatPort)
    ...
    def launch(self):
        if not self.walletd:
            Utils.Print("ERROR: Wallet Manager wasn't configured to launch keyseatd")
            return False

        cmd="%s --data-dir %s --config-dir %s --http-server-address=%s:%d --verbose-http-errors" % (
            Utils.SeatWalletPath, WalletMgr.__walletDataDir, WalletMgr.__walletDataDir, self.host, self.port)
    ...
    def create(self, name, accounts=None, exitOnError=True):
        ...
        if wallet is not None:
            ...
            return wallet
        p = re.compile(r'\n\"(\w+)\"\n', re.MULTILINE)
        cmd="%s %s wallet create --name %s --to-console" % (Utils.SeatClientPath, self.endpointArgs, name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def importKey(self, account, wallet, ignoreDupKeyWarning=False):
        ...
        cmd="%s %s wallet import --name %s --private-key %s" % (
            Utils.SeatClientPath, self.endpointArgs, wallet.name, account.ownerPrivateKey)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
        ...
        if account.activePrivateKey is None:
            ...
            cmd="%s %s wallet import --name %s  --private-key %s" % (
                Utils.SeatClientPath, self.endpointArgs, wallet.name, account.activePrivateKey)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            try:
                Utils.checkOutput(cmd.split())
            except subprocess.CalledProcessError as ex:
    ...
    def lockWallet(self, wallet):
        cmd="%s %s wallet lock --name %s" % (Utils.SeatClientPath, self.endpointArgs, wallet.name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def unlockWallet(self, wallet):
        cmd="%s %s wallet unlock --name %s" % (Utils.SeatClientPath, self.endpointArgs, wallet.name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def lockAllWallets(self):
        cmd="%s %s wallet lock_all" % (Utils.SeatClientPath, self.endpointArgs)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
    ...
    def getOpenWallets(self):
        ...
        cmd="%s %s wallet list" % (Utils.SeatClientPath, self.endpointArgs)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        retStr=Utils.checkOutput(cmd.split())
    ...
    def getKeys(self, wallet):
        ...
        p = re.compile(r'\n\s+\"(\w+)\"\n', re.MULTILINE)
        cmd="%s %s wallet private_keys --name %s --password %s " % (Utils.SeatClientPath, self.endpointArgs, wallet.name, wallet.password)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        retStr=Utils.checkOutput(cmd.split())
        #Utils.Print("retStr: %s" % (retStr))
        m=p.findall(retStr)
    ...
    def killall(self, allInstances=False):
        """Kill kseat instances. allInstances will kill all kseat instances running on the system."""
        if self.__walletPid:
            Utils.Print("Killing wallet manager process %d" % (self.__walletPid))
            os.kill(self.__walletPid, signal.SIGKILL)

        if allInstances:
            cmd="pkill -9 %s" % (Utils.SeatWalletName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
