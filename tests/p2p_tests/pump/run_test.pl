...
my $seat_home = defined $ENV{SEAT_HOME} ? $ENV{SEAT_HOME} : getcwd;
my $seatd = $seat_home . "/programs/seatd/seatd";
my $seatc = $seat_home . "/programs/seatc/seatc";

my $nodes = defined $ENV{SEAT_TEST_RING} ? $ENV{SEAT_TEST_RING} : "1";
my $pnodes = defined $ENV{SEAT_TEST_PRODUCERS} ? $ENV{SEAT_TEST_PRODUCERS} : "1";

my $prods = 21;
my $genesis = "$seat_home/genesis.json";
...
if (!GetOptions("nodes=i" => \$nodes,
    ...
    print "where:\n";
    print "--nodes=n (default = 1) sets the number of seatd instances to launch\n";
...
sub write_config {
    ...
    if (defined $producer) {
        ...
        print $cfg "private-key = [\"SEAT6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\",\"5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3\"]\n";

        print $cfg "plugin = cubetrain::producer_plugin\n";
        print $cfg "plugin = cubetrain::chain_api_plugin\n";
...
sub perform_work {
    ...
    if ($mode == 0) {
        ...
        my $counter = 0;
        while (time () < $stoptime) {
            `$seatc transfer seat inita 10 >> seatc.out 2>> seatc.err`;
            $counter++;
            if ($counter % 1000 == 0) {
