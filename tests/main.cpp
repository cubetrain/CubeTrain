...
//extern uint32_t SEAT_TESTING_GENESIS_TIMESTAMP;
...
boost::unit_test::test_suite* init_unit_test_suite(int argc, char* argv[]) {
   ...
   /*
   const char* genesis_timestamp_str = getenv("SEAT_TESTING_GENESIS_TIMESTAMP");
   if( genesis_timestamp_str != nullptr )
   {
      SEAT_TESTING_GENESIS_TIMESTAMP = std::stoul( genesis_timestamp_str );
   }
   std::cout << "SEAT_TESTING_GENESIS_TIMESTAMP is " << SEAT_TESTING_GENESIS_TIMESTAMP << std::endl;
   */
   return nullptr;
}
