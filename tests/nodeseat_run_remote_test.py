...
###############################################################
# nodeseat_run_remote_test
#  Tests remote capability of the nodeseat_run_test. Test will setup cluster and pass nodes info to nodeseat_run_test. E.g.
#  nodeseat_run_remote_test.py -v --clean-run --dump-error-detail
###############################################################
...
try:
    ...
    Print("Starting up %s test: %s" % ("nodeseat", actualTest))
    Print("cmd: %s\n" % (cmd))
    ...
    TestHelper.shutdown(cluster, None, testSuccessful, killSeatInstances, False, False, killAll, dumpErrorDetails)
