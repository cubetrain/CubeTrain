...
def stageScenario(stagedNodeInfos):
    ...
    for stagedNodeInfo in stagedNodeInfos:
        configPath=os.path.join(stagingDir, "etc/cubetrain/node_%02d" % (count))
...
killSeatInstances= not args.dont-kill
...
def myTest(transWillEnterBlock):
    ...
    try:
        ...
        if cluster.launch(pnodes, total_nodes, topo, delay) is False:
            error("Failed to stand up seat cluster.")
            return False
        ...
        if walletMgr.launch() is False:
            error("Failed to stand up seat walletd.")
            return False
