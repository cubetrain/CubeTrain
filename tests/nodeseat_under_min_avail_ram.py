...
###############################################################
# nodeseat_voting_test
# --dump-error-details <Upon error print etc/cubetrain/node_*/config.ini and var/lib/node_*/stderr.log to stdout>
...
try:
    ...
    maxRAMValue=1010
    extraNodseatArgs=" %s %d %s %d " % (minRAMFlag, minRAMValue, maxRAMFlag, maxRAMValue)
    if cluster.launch(onlyBios=False, dontKill=dontKill, pnodes=totalNodes, totalNodes=totalNodes, totalProducers=totalNodes, extraNodseatArgs=extraNodseatArgs) is False:
    ...
    if walletMgr.launch() is False:
        ...
        errorExit("Failed to stand up seat walletd.")

    testWallet=walletMgr.create(testWalletName, [cluster.cubetrainAccount])
    ...
    # create accounts via cubetrain as otherwise a bid is needed
    for account in accounts:
        Print("Create new account %s via %s" % (account.name, cluster.cubetrainAccount.name))
        trans=nodes[0].createInitializeAccount(account, cluster.cubetrainAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
        transferAmount="70000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.cubetrainAccount.name, account.name))
        nodes[0].transferFunds(cluster.cubetrainAccount, account, transferAmount, "test transfer")
    ...
    Print("Create new account %s via %s" % (contractAccount.name, cluster.cubetrainAccount.name))
    trans=nodes[0].createInitializeAccount(contractAccount, cluster.cubetrainAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
    transferAmount="90000000.0000 {0}".format(CORE_SYMBOL)
    Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.cubetrainAccount.name, contractAccount.name))
    nodes[0].transferFunds(cluster.cubetrainAccount, contractAccount, transferAmount, "test transfer")
        ...
        for fromIndex in range(namedAccounts.numAccounts):
            ...
            try:
                ...
                if trans is None or not trans[0]:
                    Print("Failed to push create action to cubetrain contract. sleep for 60 seconds")
                    time.sleep(60)
                time.sleep(1)
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killSeatInstances, killWallet, keepLogs, killAll, dumpErrorDetails)
