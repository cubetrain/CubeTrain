...
try:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.cubetrainAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    ...
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.cubetrainAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.cubetrainAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.cubetrainAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.cubetrainAccount, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.cubetrainAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.cubetrainAccount, waitForTransBlock=True, exitOnError=True)
    ...
    actualAmount=node.getAccountSeatBalanceStr(testeraAccount.name)
    ...
    Print("Verify transfer, Expected: %s" % (expectedAmount))
    actualAmount=node.getAccountSeatBalanceStr(currencyAccount.name)
    if expectedAmount != actualAmount:
        cmdError("FAILURE - transfer failed")
        errorExit("Transfer verification failed. Excepted %s, actual: %s" % (expectedAmount, actualAmount))
    ...
    contractDir="contracts/cubetrain.tk"
    wasmFile="cubetrain.tk.wasm"
    abiFile="cubetrain.tk.abi"
    ...
    if not enableMongo:
        ...
        account=node.getSeatAccountFromDb(currencyAccount.name)
        ...
        if abiName != "transfer" or abiActionName != "transfer" or abiType != "transfer":
            errorExit("FAILURE - get SEAT account failed", raw=True)
    ...
    account=node.getSeatAccount(defproduceraAccount.name, exitOnError=True)
