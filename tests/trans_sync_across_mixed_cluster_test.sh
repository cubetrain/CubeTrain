...
killAll()
{
  programs/cubetrain-launcher/cubetrain-launcher -k 15
}

cleanup()
{
    rm -rf etc/cubetrain/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/cliseat/cliseat get info)"
  verifyErrorCode "cliseat get info"
...
# stand up nodeseat cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodeseat \"--plugin cubetrain::wallet_api_plugin\" $launcherOpts
programs/cubetrain-launcher/cubetrain-launcher --nodeseat "--plugin cubetrain::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/cliseat/cliseat --port $port2 get block 1)"
    verifyErrorCode "cliseat get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/cliseat/cliseat create key)"
verifyErrorCode "cliseat create key"
...
PASSWORD_INITA="$(programs/cliseat/cliseat wallet create --name inita)"
verifyErrorCode "cliseat wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/cliseat/cliseat wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "cliseat wallet import"
programs/cliseat/cliseat wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "cliseat wallet import"
programs/cliseat/cliseat wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "cliseat wallet import"
programs/cliseat/cliseat wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "cliseat wallet import"
...
ACCOUNT_INFO="$(programs/cliseat/cliseat create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "cliseat create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/cliseat/cliseat get account testera)"
verifyErrorCode "cliseat get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/cliseat/cliseat transfer inita testera 975321 "test transfer")"
    verifyErrorCode "cliseat transfer"
   ...
   TRANS_INFO="$(programs/cliseat/cliseat --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "cliseat get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
