...
class Node(object):
    ...
    def __init__(self, host, port, pid=None, cmd=None, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="SEATtest"):
        self.host=host
        self.port=port
        self.pid=pid
        self.cmd=cmd
    ...
    def validateAccounts(self, accounts):
        ...
        for account in accounts:
            ...
            accountInfo=self.getSeatAccount(account.name, exitOnError=True)
            try:
    ...
    def getSeatAccount(self, name, exitOnError=False):
        assert(isinstance(name, str))
        if not self.enableMongo:
            cmdDesc="get account"
            cmd="%s -j %s" % (cmdDesc, name)
            msg="( getSeatAccount(name=%s) )" % (name);
            return self.processCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError, exitMsg=msg)
        else:
            return self.getSeatAccountFromDb(name, exitOnError=exitOnError)

    def getSeatAccountFromDb(self, name, exitOnError=False):
        cmd="%s %s" % (Utils.MongoPath, self.mongoEndpointArgs)
    ...
    def verifyAccount(self, account):
        assert(account)
        if not self.enableMongo:
            ret=self.getSeatAccount(account.name)
            if ret is not None:
    ...
    def verifyAccountMdb(self, account):
        assert(account)
        ret=self.getSeatAccountFromDb(account.name)
        if ret is not None:
    ...
    def transferFunds(self, source, destination, amountStr, memo="memo", force=False, waitForTransBlock=False, exitOnError=True):
        ...
        cmd="%s %s -v transfer -j %s %s" % (
            Utils.SeatClientPath, self.endpointArgs, source.name, destination.name)
        cmdArr=cmd.split()
    ...
    def currencyStrToInt(balanceStr):
        """Converts currency string of form "12.3456 SEAT" to int 123456"""
    ...
    def currencyIntToStr(balance, symbol):
        """Converts currency int of form 123456 to string "12.3456 SEAT" where SEAT is symbol string"""
    ...
    def validateFunds(self, initialBalances, transferAmount, source, accounts):
        """Validate each account has the expected SEAT balance. Validate cumulative balance matches expectedTotal."""
        assert(source)
        assert(isinstance(source, Account))
        assert(accounts)
        assert(isinstance(accounts, list))
        ...
        currentBalances=self.getSeatBalances([source] + accounts)
    ...
    def getSeatBalances(self, accounts):
        ...
        balances={}
        for account in accounts:
            balance = self.getAccountSeatBalance(account.name)
    ...
    # Get actions mapped to an account (cliseat get actions)
    ...
    def getAccountSeatBalanceStr(self, scope):
        """Returns SEAT currency0000 account balance from cliseat get table command. Returned balance is string following syntax "98.0311 SEAT". """
        assert isinstance(scope, str)
        amount=self.getTableAccountBalance("cubetrain.tk", scope)
        if Utils.Debug: Utils.Print("getNodeAccountSeatBalance %s %s" % (scope, amount))
        assert isinstance(amount, str)
        return amount

    def getAccountSeatBalance(self, scope):
        """Returns SEAT currency0000 account balance from cliseat get table command. Returned balance is an integer e.g. 980311. """
        balanceStr=self.getAccountSeatBalanceStr(scope)
        balance=Node.currencyStrToInt(balanceStr)
        return balance

    def getAccountCodeHash(self, account):
        cmd="%s %s get code %s" % (Utils.SeatClientPath, self.endpointArgs, account)
    ...
    def publishContract(self, account, contractDir, wasmFile, abiFile, waitForTransBlock=False, shouldFail=False):
        cmd="%s %s -v set contract -j %s %s" % (Utils.SeatClientPath, self.endpointArgs, account, contractDir)
    ...
    def pushMessage(self, account, action, data, opts, silentErrors=False):
        cmd="%s %s push action -j %s %s" % (Utils.SeatClientPath, self.endpointArgs, account, action)
        cmdArr=cmd.split()
        if data is not None:
            cmdArr.append(data)
        if opts is not None:
    ...
    def processCmd(self, cmd, cmdDesc, silentErrors=True, exitOnError=False, exitMsg=None, returnType=ReturnType.json):
        ...
        cmd="%s %s %s" % (Utils.SeatClientPath, self.endpointArgs, cmd)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        trans=None
        try:
    ...
    def getHeadBlockNum(self):
        """returns head block number(string) as returned by cliseat get info."""
        if not self.enableMongo:
