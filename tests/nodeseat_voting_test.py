...
try:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.cubetrainAccount,accounts[0],accounts[1],accounts[2],accounts[3],accounts[4]])
    ...
    for account in accounts:
        ...
        trans=node.createInitializeAccount(account, cluster.cubetrainAccount, stakedDeposit=0, waitForTransBlock=False, stakeNet=1000, stakeCPU=1000, buyRAM=1000, exitOnError=True)
        transferAmount="100000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.cubetrainAccount.name, account.name))
        node.transferFunds(cluster.cubetrainAccount, account, transferAmount, "test transfer")
