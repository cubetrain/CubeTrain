...
class Cluster(object):
    ...
    # walletd [True|False] Is keyseatd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="SEATtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet keyseatd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: seat server host
        port: seat server port
        walletHost: seat wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures seat mongo plugin
        ...
        self.cubetrainAccount=self.defProducerAccounts["cubetrain"]= Account("cubetrain")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontKill=False
               , dontBootstrap=False, totalProducers=None, extraNodseatArgs=None):
        """Launch cluster.
        ...
        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.SeatServerName))
            return True
        ...
        while not Cluster.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodeseat default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        cmd="%s -p %s -n %s -s %s -d %s -i %s -f --p2p-plugin %s %s" % (
            Utils.SeatLauncherPath, pnodes, totalNodes, topo, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            p2pPlugin, producerFlag)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        nodeseatArgs="--max-transaction-time 50000 --abi-serializer-max-time-ms 990000 --filter-on * --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodeseatArgs += " --plugin cubetrain::wallet_api_plugin"
        if self.enableMongo:
            nodeseatArgs += " --plugin cubetrain::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodseatArgs is not None:
            assert(isinstance(extraNodseatArgs, str))
            nodeseatArgs += extraNodseatArgs
        if Utils.Debug:
            nodeseatArgs += " --contracts-console"

        if nodeseatArgs:
            cmdArr.append("--nodeseat")
            cmdArr.append(nodeseatArgs)
        ...
        if nodes is None or totalNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.SeatServerName, totalNodes, len(nodes)))
            return False
        ...
        self.cubetrainAccount=self.defProducerAccounts["cubetrain"]
    ...
    def getClientVersion(verbose=False):
        ...
        try:
            cmd="%s version client" % (Utils.SeatClientPath)
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key --to-console" % (Utils.SeatClientPath)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on seat server port %d" % (
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nextSeatIdx: %d, n: %d" % (nextSeatIdx, n))
                nextSeatIdx=(nextSeatIdx + 1)%count
                if not self.nodes[nextSeatIdx].killed:
                    #Utils.Print("nextSeatIdx: %d" % (nextSeatIdx))
            ...
            #Utils.Print("nextSeatIdx: %d, count: %d" % (nextSeatIdx, count))
            node=self.nodes[nextSeatIdx]
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.SeatServerName, node.port))

            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on seat node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        receiving transferAmount*n SEAT and forwarding x-transferAmount funds. Transfer actions are spread round-robin across the cluster to vaidate system cohesiveness."""

        if Utils.Debug: Utils.Print("Get initial system balances.")
        initialBalances=self.nodes[0].getSeatBalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.cubetrainAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseProducers(nodeNum):
        ...
        configFile="etc/cubetrain/%s/config.ini" % (node)
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse cubetrain private keys from cluster config files.")
            return None
    ...
    def bootstrap(totalNodes, prodCount, biosHost, biosPort, dontKill=False, onlyBios=False):
        """Create 'prodCount' init accounts and deposits 10000000000 SEAT in each. If prodCount is -1 will initialize all possible producers.
        ...
        try:
            ...
            cubetrainName="cubetrain"
            cubetrainKeys=producerKeys[cubetrainName]
            cubetrainAccount=Account(cubetrainName)
            cubetrainAccount.ownerPrivateKey=cubetrainKeys["private"]
            cubetrainAccount.ownerPublicKey=cubetrainKeys["public"]
            cubetrainAccount.activePrivateKey=cubetrainKeys["private"]
            cubetrainAccount.activePublicKey=cubetrainKeys["public"]

            if not walletMgr.importKey(cubetrainAccount, ignWallet):
                Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (cubetrainName))
                return None

            contract="cubetrain.bios"
            contractDir="contracts/%s" % (contract)
            wasmFile="%s.wasm" % (contract)
            abiFile="%s.abi" % (contract)
            Utils.Print("Publish %s contract" % (contract))
            trans=biosNode.publishContract(cubetrainAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
            if trans is None:
            ...
            producerKeys.pop(cubetrainName)
            accounts=[]
            for name, keys in producerKeys.items():
                ...
                trans=biosNode.createAccount(initx, cubetrainAccount, 0)
                if trans is None:
            ...
            if not onlyBios:
                if prodCount == -1:
                    ...
                    with open(setProdsFile, "r") as f:
                        ...
                        myTrans=biosNode.pushMessage("cubetrain", "setprods", setProdsStr, opts)
                    ...
                    trans=biosNode.pushMessage("cubetrain", "setprods", setProdsStr, opts)
                ...
                # wait for block production handover (essentially a block produced by anyone but cubetrain).
                lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "cubetrain"
                ret=Utils.waitForBool(lam)
                if not ret:
                    Utils.Print("ERROR: Block production handover failed.")
                    return None

            cubetrainTokenAccount=copy.deepcopy(cubetrainAccount)
            cubetrainTokenAccount.name="cubetrain.tk"
            trans=biosNode.createAccount(cubetrainTokenAccount, cubetrainAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (cubetrainTokenAccount.name))
                return None

            cubetrainRamAccount=copy.deepcopy(cubetrainAccount)
            cubetrainRamAccount.name="cubetrain.rm"
            trans=biosNode.createAccount(cubetrainRamAccount, cubetrainAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (cubetrainRamAccount.name))
                return None

            cubetrainRamfeeAccount=copy.deepcopy(cubetrainAccount)
            cubetrainRamfeeAccount.name="cubetrain.rf"
            trans=biosNode.createAccount(cubetrainRamfeeAccount, cubetrainAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (cubetrainRamfeeAccount.name))
                return None

            cubetrainStakeAccount=copy.deepcopy(cubetrainAccount)
            cubetrainStakeAccount.name="cubetrain.st"
            trans=biosNode.createAccount(cubetrainStakeAccount, cubetrainAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (cubetrainStakeAccount.name))
                return None
            ...
            contract="cubetrain.tk"
            contractDir="contracts/%s" % (contract)
            wasmFile="%s.wasm" % (contract)
            abiFile="%s.abi" % (contract)
            Utils.Print("Publish %s contract" % (contract))
            trans=biosNode.publishContract(cubetrainTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
            if trans is None:
                Utils.Print("ERROR: Failed to publish contract %s." % (contract))
                return None

            # Create currency0000, followed by issue currency0000
            contract=cubetrainTokenAccount.name
            Utils.Print("push create action to %s contract" % (contract))
            action="create"
            data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\",\"can_freeze\":\"0\",\"can_recall\":\"0\",\"can_whitelist\":\"0\"}" % (cubetrainTokenAccount.name, CORE_SYMBOL)
            opts="--permission %s@active" % (contract)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to push create action to cubetrain contract.")
                return None
            ...
            action="issue"
            data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (cubetrainAccount.name, CORE_SYMBOL)
            opts="--permission %s@active" % (contract)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to push issue action to cubetrain contract.")
                return None
            ...
            Utils.Print("Verify cubetrain issue, Expected: %s" % (expectedAmount))
            actualAmount=biosNode.getAccountSeatBalanceStr(cubetrainAccount.name)
            if expectedAmount != actualAmount:
                Utils.Print("ERROR: Issue verification failed. Excepted %s, actual: %s" %
                            (expectedAmount, actualAmount))
                return None

            contract="cubetrain.system"
            contractDir="contracts/%s" % (contract)
            wasmFile="%s.wasm" % (contract)
            abiFile="%s.abi" % (contract)
            Utils.Print("Publish %s contract" % (contract))
            ...
            action="transfer"
            for name, keys in producerKeys.items():
                data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (cubetrainAccount.name, name, initialFunds, "init transfer")
                opts="--permission %s@active" % (cubetrainAccount.name)
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (cubetrainTokenAccount.name, name))
                    return None
    ...
    # Populates list of SeatInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=0):
        ...
        cmd="pgrep %s %s" % (pgrepOpts, Utils.SeatServerName)
        ...
        for i in range(0, totalNodes):
            ...
            if m is None:
                Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.SeatServerName, pattern))
                break
    ...
    # Kills a percentange of Seat instances starting from the tail and update seatInstanceInfos state
    def killSomeSeatInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.SeatServerName, killSignal))
    ...
    def relaunchSeatInstances(self):
    ...
    def dumpErrorDetails(self):
        fileName="etc/cubetrain/node_bios/config.ini"
        Cluster.dumpErrorDetailImpl(fileName)
        fileName="var/lib/node_bios/stderr.txt"
        Cluster.dumpErrorDetailImpl(fileName)

        for i in range(0, len(self.nodes)):
            fileName="etc/cubetrain/node_%02d/config.ini" % (i)
            Cluster.dumpErrorDetailImpl(fileName)
            fileName="var/lib/node_%02d/stderr.txt" % (i)
            Cluster.dumpErrorDetailImpl(fileName)

    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodeseat instances. allInstances will kill all nodeseat instances running on the system."""
        cmd="%s -k 9" % (Utils.SeatLauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down seat cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the seat server
            cmd="pkill -9 %s" % (Utils.SeatServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down seat cluster.")
    ...
    def cleanup(self):
        for f in glob.glob("var/lib/node_*"):
            shutil.rmtree(f)
        for f in glob.glob("etc/cubetrain/node_*"):
            shutil.rmtree(f)
