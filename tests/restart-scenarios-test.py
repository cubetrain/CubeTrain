...
# --kill-count <nodeseat instances to kill>
...
try:
    ...
    Print("Stand up SEAT wallet keyseatd")
    walletMgr.killall(allInstances=killAll)
    walletMgr.cleanup()
    if walletMgr.launch() is False:
        errorExit("Failed to stand up keyseatd.")

    accountsCount=total_nodes
    walletName="MyWallet"
    Print("Creating wallet %s if one doesn't already exist." % walletName)
    wallet=walletMgr.create(walletName, [cluster.cubetrainAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    ...
    cubetrainAccount=cluster.cubetrainAccount
    ...
    if not cluster.createAccounts(cubetrainAccount):
    ...
    if cluster.killSomeSeatInstances(killCount, killSignal) is False:
        errorExit("Failed to kill Seat instances")
    Print("nodeseat instances killed.")
    ...
    if cluster.relaunchSeatInstances() is False:
        errorExit("Failed to relaunch Seat instances")
    Print("nodeseat instances relaunched.")
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killSeatInstances, killSeatInstances, keepLogs, killAll, dumpErrorDetails)
