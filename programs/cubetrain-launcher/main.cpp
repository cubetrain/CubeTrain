...
void
seatd_def::set_host( host_def* h, bool is_bios ) {
  host = h->host_name;
...
struct server_name_def {
  ...
  string cubetrain_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), cubetrain_home(), instances(1) {}
};
...
struct launcher_def {
  ...
  producer_set_def producer_set;
   ...
   void assign_name (seatd_def &node, bool is_bios);
  ...
  void prep_remote_config_dir (seatd_def &node, host_def *host);
  void launch (seatd_def &node, string &gts);
  void kill (launch_modes mode, string sig_opt);
  pair<host_def, seatd_def> find_node(uint16_t node_num);
  vector<pair<host_def, seatd_def>> get_nodes(const string& node_number_list);
...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    ("skip-signature", bpo::bool_switch(&skip_transaction_signatures)->default_value(false), "nodeseat does not require transaction signatures.")
    ("nodeseat", bpo::value<string>(&seatd_extra_args), "forward nodeseat command line argument(s) to each instance of nodeseat, enclose arg in quotes")
    ...
    ("per-host",bpo::value<int>(&per_host)->default_value(0),"specifies how many nodeseat instances will run on a single host. Use 0 to indicate all on one.")
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &seatd : binding.instances) {
          seatd.host = binding.host_name;
          seatd.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(seatd.p2p_port);

          aliases.push_back (seatd.name);
        }
      }
  ...
  char* erd_env_var = getenv ("CUBETRAIN_HOME");
  ...
  stage = bfs::path(erd);
  if (!bfs::exists(stage)) {
    cerr << "\"" << erd << "\" is not a valid path. Please ensure environment variable CUBETRAIN_HOME is set to the build path." << endl;
    exit (-1);
  }
...
void
launcher_def::assign_name (seatd_def &node, bool is_bios) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.cubetrain_home = erd;
    local_host.genesis = genesis.string();
    for (size_t i = 0; i < (total_nodes); i++) {
      seatd_def seatd;
      assign_name(seatd, i == 0);
      aliases.push_back(seatd.name);
      seatd.set_host (&local_host, i == 0);
      local_host.instances.emplace_back(move(seatd));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->cubetrain_home =
          (local_id.contains (lhost->host_name) || servers.default_cubetrain_home.empty()) ?
          erd : servers.default_cubetrain_home;
        host_ndx++;
      } // ph_count == 0

      seatd_def seatd;
      assign_name(seatd, do_bios);
      seatd.has_db = false;
      ...
      aliases.push_back(seatd.name);
      seatd.set_host (lhost, do_bios);
      do_bios = false;
      lhost->instances.emplace_back(move(seatd));
      --ph_count;
    } // for i
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::deploy_config_files (tn_node_def &node) {
  boost::system::error_code ec;
  seatd_def &instance = *node.instance;
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->cubetrain_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->cubetrain_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->cubetrain_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->cubetrain_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->cubetrain_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "plugin = cubetrain::producer_plugin\n";
  }
  if( instance.has_db ) {
    cfg << "plugin = cubetrain::mongo_db_plugin\n";
  }
  if ( p2p == p2p_plugin::NET ) {
    cfg << "plugin = cubetrain::net_plugin\n";
  } else {
    cfg << "plugin = cubetrain::bnet_plugin\n";
  }
  cfg << "plugin = cubetrain::chain_api_plugin\n"
...
launcher_def::init_genesis () {
  bfs::path genesis_path = bfs::current_path() / "genesis.json";
   ...
   if (!src.good()) {
      ...
      cubetrain::chain::genesis_state default_genesis;
...
void
launcher_def::prep_remote_config_dir (seatd_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->cubetrain_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->cubetrain_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->cubetrain_home;

  cmd = "cd " + host->cubetrain_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->cubetrain_home
...
void
launcher_def::launch (seatd_def &instance, string &gts) {
  ...
  string seatdcmd = "programs/nodeseat/nodeseat ";
  if (skip_transaction_signatures) {
    seatdcmd += "--skip-transaction-signatures ";
  }
  if (!seatd_extra_args.empty()) {
    if (instance.name == "bios") {
       ...
       regex r("--plugin +cubetrain::mongo_db_plugin");
       string args = std::regex_replace (seatd_extra_args,r,"");
       regex r2("--mongodb-uri +[^ ]+");
       args = std::regex_replace (args,r2,"");
       seatdcmd += args + " ";
    }
    else {
       seatdcmd += seatd_extra_args + " ";
    }
  }

  if( add_enable_stale_production ) {
    seatdcmd += "--enable-stale-production true ";
    add_enable_stale_production = false;
  }

  seatdcmd += " --config-dir " + instance.config_dir_name + " --data-dir " + instance.data_dir_name;
  seatdcmd += " --genesis-json " + instance.config_dir_name + "/genesis.json";
  if (gts.length()) {
    seatdcmd += " --genesis-timestamp " + gts;
  }

  if (!host->is_local()) {
    string cmdl ("cd ");
    cmdl += host->cubetrain_home + "; nohup " + seatdcmd + " > "
    ...
    string cmd = "cd " + host->cubetrain_home + "; kill -15 $(cat " + pidf.string() + ")";
    format_ssh (cmd, host->host_name, info.kill_cmd);
  }
  else {
    cerr << "spawning child, " << seatdcmd << endl;

    bp::child c(seatdcmd, bp::std_out > reout, bp::std_err > reerr );
    bfs::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << seatdcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
...
#if 0
void
launcher_def::kill_instance(seatd_def, string sig_opt) {
}
#endif
...
vector<pair<host_def, seatd_def>>
launcher_def::get_nodes(const string& node_number_list) {
   vector<pair<host_def, seatd_def>> node_list;
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      const seatd_def& node = node_pair.second;
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.cubetrain_home + "; "
                 + "export CUBETRAIN_HOME=" + host.cubetrain_home + string("; ")
                 + "export CUBETRAIN_NODE=" + node_num + "; "
                 + "./scripts/cubetrain-tn_bounce.sh " + seatd_extra_args;
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.cubetrain_home + "; "
                 + "export CUBETRAIN_HOME=" + host.cubetrain_home + "; "
                 + "export CUBETRAIN_NODE=" + node_num + "; "
         + "export CUBETRAIN_TN_RESTART_CONFIG_DIR=" + node.config_dir_name + "; "
                 + "./scripts/cubetrain-tn_down.sh";
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the cubetrain-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the cubetrain-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the cubetrain-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << cubetrain::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( host_def,
            (genesis)(ssh_identity)(ssh_args)(cubetrain_home)
            (host_name)(public_name)
            (base_p2p_port)(base_http_port)(def_file_size)
            (instances) )

FC_REFLECT( seatd_def,
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (cubetrain_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_cubetrain_home) (ssh) )
