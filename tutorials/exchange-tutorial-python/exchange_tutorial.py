...
def monitor_exchange():
    ...
    results = cliseat('get actions tokenxchange {} 0 -j'.format(action_num))
...
def transfer(to, quantity):
    if quantity[:-4] != ' SEAT':
        quantity += ' SEAT'
    results = cliseat('transfer tokenxchange {} "{}" {} -j'.format(to, quantity, KEY_TO_INTERNAL_ACCOUNT))
